//Primitivas de Geometria Computacional Discreta com variáveis inteiras
// CONVENÇÕES:
...
/*   Ordena pontos por ângulo, escolhendo como referência o ponto mais embaixo e mais
   à esquerda. */
	...
	m=0;
	for (i=1;i<tp;i++) 
		if ( (pol[i].y < pol[m].y) || (pol[i].y==pol[m].y) && (pol[i].x<pol[m].x) ) 
			m=i;
...
/* Ordena pontos por ângulo, escolhendo como referência o ponto mais à esquerda e mais
   embaixo. */
	...
	m=0;
	for (i=1;i<tp;i++) 
		if ( (pol[i].x < pol[m].x) || (pol[i].x==pol[m].x) && (pol[i].y<pol[m].y) )
			m=i;
	p=pol[0];   pol[0]=pol[m];   pol[m]=p;
	Quicksort(pol, 1,tp-1);
}
...
{  int i,m;  Ponto p;
	...
	i = tp-1;  
	while ((i>0)&&(SentidoPercurso(pol[0],pol[i-1],pol[i])==0))
		i--;
	for (m=i; m<(i+tp)/2;m++) {
		p=pol[m]; pol[m]=pol[tp-1-m+i]; pol[tp-1-m+i]=p;
	}
}
...
int ConvexHull(Ponto *pol, int tp, Ponto *ch){
    ...
    //Empilha 2 pontos iniciais
    ch[0] = pol[0];  ch[1] = pol[1];  topo = 1;
    
    //Graham Scan
    for (i= 2; i<= tp; i++){
		while (SentidoPercurso(ch[topo-1], ch[topo], pol[i]) != 1  && topo > 0) 
			topo--;
        topo++;  ch[topo]= pol[i];
    }
    return (topo);
...
void ExemploContornoConvexo(){
    ...
    nv = ConvexHull(pol, np, ch);
    cout<<"Contorno Convexo com " <<nv <<" pontos:";
